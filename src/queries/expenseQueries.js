const CREATE_NEW_EXPENSE = `INSERT INTO visa (electricity,telephone,salary,stationary,other,createdUser, updatedUser,remarks) 
...
const GET_ALL_EXPENSE_QUERY = `SELECT DISTINCT ON(createdTime) id,total_amount,fileId,electricity,telephone,salary,stationary,other,remarks,createdTime
    FROM visa WHERE createdTime ILIKE '%' || $1 || '%'  LIMIT 100`
...
const UPDATE_EXPENSE = `UPDATE visa SET electricity=$2,telephone=$3,salary=$4,stationary=$5,other=$6,createdUser=$7, remarks=$8 WHERE id = $1`


const DELETE_EXPENSE = `DELETE FROM visa WHERE id = $1`
